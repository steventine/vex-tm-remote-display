...
"#status-dot { position: absolute; top: 6px; right: 6px; width: 6px; height: 6px; border-radius: 50%; background: #ef4444; z-index: 10; transition: background 0.4s; }\n"
"</style>\n"
"</head>\n"
"<body>\n"
...
"\n"
"// Status dot — blinks at 2s period (1000ms on/off) when active, solid red when stale.\n"
"// Use video timeupdate as the liveness signal: fires ~4x/sec while playing,\n"
"// goes quiet immediately when playback stalls, recovers within one frame.\n"
"var lastUpdateMs = 0, currentLatency = 0, dotOn = true;\n"
"var STALE_MS = 3000; // red if video hasn't progressed in 3s\n"
...
"    return;\n"
"  }\n"
"  var bg = currentLatency < 5 ? '#22c55e' : (currentLatency < 8 ? '#eab308' : '#f97316');\n"
...
"        if (latency > 0) { currentLatency = latency; statLatency.textContent = 'Latency: ' + latency.toFixed(1) + 's'; }\n"
"      }\n"
"    }, 1000);\n"
